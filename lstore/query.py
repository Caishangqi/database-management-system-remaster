...
class Query:
    ...
    def select(self, key, key_index, query_columns):
        if len(self.table.base_list) < 1:
            return []
        records = []

        if key_index == self.table.key:
            ...
            rid_list = []
            for bp in self.table.base_list:
                for j in range(0, bp.physical_pages[0].num_records):
                    if key == bp.physical_pages[4 + key_index].read(j):
                        if bp.physical_pages[0].read(j) == 0:
                            rid_list.append(bp.physical_pages[1].read(j))
                        else:
                            rid_list.append(bp.physical_pages[0].read(j))
    ...
    def update(self, key, *columns):
        ...
        if check_indirection == 0:
            ...
            self.table.base_list[indirection_location[0]].physical_pages[3].update(1, indirection_location[1])
            # set schema fields to 1
